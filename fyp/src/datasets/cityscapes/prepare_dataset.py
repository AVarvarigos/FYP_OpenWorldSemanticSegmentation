...
if __name__ == "__main__":
    ...
    label_filepaths = [
        fp for fp in label_filepaths if os.path.basename(fp).find("labelIds") > -1
    ]
    ...
    for rgb_fp in tqdm(rgb_filepaths):
        ...
        dest_path = os.path.join(args.output_path, subset, CityscapesBase.RGB_DIR, city)
    ...
    mapping_1plus33_to_1plus19 = np.array(
        [CityscapesBase.CLASS_MAPPING_REDUCED[i] for i in range(1 + 33)], dtype="uint8"
    )
    ...
    for l_fp in tqdm(label_filepaths):
        ...
        dest_path = os.path.join(
            args.output_path, subset, CityscapesBase.LABELS_FULL_DIR, city
        )
        os.makedirs(dest_path, exist_ok=True)
        ...
        dest_path = os.path.join(
            args.output_path, subset, CityscapesBase.LABELS_FULL_COLORED_DIR, city
        )
        os.makedirs(dest_path, exist_ok=True)
        save_indexed_png(
        ...
        dest_path = os.path.join(
            args.output_path, subset, CityscapesBase.LABELS_REDUCED_DIR, city
        )
        os.makedirs(dest_path, exist_ok=True)
        ...
        dest_path = os.path.join(
            args.output_path, subset, CityscapesBase.LABELS_REDUCED_COLORED_DIR, city
        )
        os.makedirs(dest_path, exist_ok=True)
        save_indexed_png(
            os.path.join(dest_path, basename),
    ...
    np.savetxt(
        os.path.join(output_path, "class_names_1+33.txt"),
        CityscapesBase.CLASS_NAMES_FULL,
        delimiter=",",
        fmt="%s",
    )
    np.savetxt(
        os.path.join(output_path, "class_colors_1+33.txt"),
