    ...
    if kwargs["block"] != BasicBlock and pretrained_on_imagenet:
        ...
        if "input_channels" in kwargs and kwargs["input_channels"] == 1:
            ...
            weights["conv1.weight"] = torch.sum(
                weights["conv1.weight"], axis=1, keepdim=True
            )
...
def load_pretrained_with_different_encoder_block(
    ...
    if not os.path.exists(ckpt_path):
        ...
        ckpt_path = os.path.join(pretrained_dir, "ckpt_epoch_{}.pth".format(epoch))
    ...
    print(
        f"Loaded {resnet_name} with encoder block {encoder_block} "
