...
train_losses = []
val_losses = []
miou_scores = []  # Example of tracking a metric (mean IoU)
...
def train_main():
    ...
    # checkpoint = torch.load("/teamspace/studios/this_studio/Open-World-Semantic-Segmentation-Including-Class-Similarity/trained_models/imagnet/r34_NBt1D.pth")
    # state_dict = checkpoint.get("state_dict", checkpoint)
    # # Filter keys to match your model
    ...
    for epoch in range(int(start_epoch), args.epochs):
        ...
        if total_train_loss < 200: # TO REMOVE HIGH SPIKES IN LOSS
            train_losses.append(total_train_loss)
        val_losses.append(miou)  # Example: track validation mIoU
        miou_scores.append(miouhere)
        ...
        if (epoch + 1) % 20 == 0:
            ...
            # Plot validation loss or mIoU
            # plot_metrics(val_losses, "Validation Loss", epochs, f"{ckpt_dir}/val_loss_epoch_{epoch + 1}.png")

            # Plot mIoU
            # plot_metrics(miou_scores, "Mean IoU", epochs, f"{ckpt_dir}/miou_epoch_{epoch + 1}.png")
